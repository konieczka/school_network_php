...
$existingTeachersQuery = pg_query($db_connection, "SELECT * FROM students WHERE first_name = '$first_name' AND last_name ='$last_name'");
$teacherAlreadyExists = pg_num_rows($existingTeachersQuery);

if ($teacherAlreadyExists > 0) {
    header('location:students.php?success=0');
} else {
    $id = hexdec(uniqid());
    $createTeacherQuery = "INSERT INTO students (student_id, first_name, last_name, date_of_birth) VALUES ($id, '$first_name', '$last_name', '$birth_date')";
